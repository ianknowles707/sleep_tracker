...
fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            if (it.endTimeMillie != it.startTimeMilli) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.endTimeMillie)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.sleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.endTimeMillie.minus(it.startTimeMilli) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.endTimeMillie.minus(it.startTimeMilli) / 1000 / 60}:")
                // Seconds
                append("${it.endTimeMillie.minus(it.startTimeMilli) / 1000}<br><br>")
            }
        }
    }
